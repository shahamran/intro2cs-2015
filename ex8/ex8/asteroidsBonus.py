...
class GameRunner:
    ...
    def __init__(self, amnt = 3):
        ...
        # for the bonus..
        self.TORPEDO_TYPE = "torpedo"
        self.MISSILE_TYPE = "missile"
    ...
    def get_close_asteroid(self):
        """
        This method finds the asteroid which is closest to the spaceship.
        :return: Asteroid (baseObject)
        """

        min_dist = self.screenMaxX - self.screenMinX
        close_asteroid = None
        ...
        for asteroid in self.game.get_asteroids():
            ...
            dist = ((a_x - x) ** 2 + (a_y - y) ** 2) ** 0.5
            if dist < min_dist:
                close_asteroid = asteroid
                min_dist = dist
        return close_asteroid

    def create_missile(self, ship,speed,factor,angle):
        """
        When the missile button is pressed, thie method adds to the game a
        heat-seeking missiles (that behaves like a torpedo, only smarter).
        
        :param ship: the spaceship
        :type ship: SpaceShip ( baseObject )
        :param speed: the speed of the spaceship
        :type speed: tuple (x_speed, y_speed)
        :param factor: (cos(angle), sin(angle))
        :type factor: tuple (factor_x, factor_y | factor_x,factor_y as float)
        :param angle: the angle of the ship
        :type angle: float
        """
        # doesn't shoot if there too many missiles
        if self.num_missiles >= self.MAX_MISSILES: return
        pos = ( ship.get_x_cor(), ship.get_y_cor() )
        asteroid = self.get_close_asteroid()
        axis_min = ( self.screenMinX, self.screenMinY )
        axis_max = ( self.screenMaxX, self.screenMaxY )

        #   V - precaution - works without it - V   #
        if asteroid == None: asteroid = self.game.get_asteroids()[0]

        tor_speed = [0] * self.DIMS
        for i in range(self.DIMS): # sets the desired missile speed
            tor_speed[i] = speed[i] + 5 * factor[i]
        self.game.add_missile(pos[0], pos[1], tor_speed[0], \
                              tor_speed[1], angle, asteroid,axis_min,axis_max)
        self.num_missiles += 1
    ...
    def _handle_key_press(self, ship, torpedos, asteroids):
        ...
        :type torpedos: list ( of PhotonTorpedo/HeatSeeker objects )
        ...
        if self.game.is_up_pressed():
            ...
            total_speed = (new_speed[0] ** 2 + new_speed[1] ** 2) ** (1/2)
        ...
        # only shoots if there are less than MAX_TORPEDOS torpedos on screen
            if len(torpedos) < self.MAX_TORPEDOS:
                ...
                self.game.add_torpedo(pos[0], pos[1], tor_speed[0], \
                                                      tor_speed[1], angle)
        # handles missile key press
        if self.game.is_missile_pressed():
            if len(torpedos) < self.MAX_TORPEDOS:
                self.create_missile(ship, speed, factor, angle)
    ...
    def game_loop(self):
        ...
        for torpedo in torpedos:
            if torpedo.get_life_span() > 0:
                # NOTE that the missile moves differently
                self._move_object(torpedo)

                is_hit = self._handle_intersection(torpedo, asteroids)
                if is_hit:
                    if torpedo.get_type() == self.MISSILE_TYPE:
                        self.num_missiles -= 1
...
def main():
    # the bonus_2 part, an input for the desired number of asteroids
    input_msg = "Insert desired number of asteroids (1 to 30): "
    amnt_ast = input(input_msg)
    while not amnt_ast.isnumeric() or int(amnt_ast) < 1 or int(amnt_ast) > 30:
        print("Illegal input! type a number between 1 to 30")
        amnt_ast = input(input_msg)

    amnt_ast = int(amnt_ast)
    
    runner = GameRunner(amnt_ast)
    runner.run()
